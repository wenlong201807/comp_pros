      ...
      <scroller :on-refresh="refresh" ref="myscroller" style="width: 100%;height: 100%;">
        <div class="topContainer" v-if="aheadRank.length!==0">
          <div class="topInner">
            <div class="left">
              ...
              <div class="brand">{{ ((aheadRank[1]||{}).vehicleName||'').substring(0,4) }}</div>
              <div class="numer">
                <span class="num">{{ (aheadRank[1]||{}).num }}</span>
            ...
            <div class="center">
              <div class="gold"></div>
              <div class="brand bigText">{{ ((aheadRank[0]||{}).vehicleName||'').substring(0,4) }}</div>
              <div class="numer bigNumTop">
                <span class="num bigNum">{{ (aheadRank[0]||{}).num }}</span>
                <span class="unit">辆</span>
              </div>
              <div class="percent">
            ...
            <div class="right">
              ...
              <div class="brand">{{ ((aheadRank[2]||{}).vehicleName||'').substring(0,4) }}</div>
              <div class="numer">
                <span class="num">{{ (aheadRank[2]||{}).num }}</span>
                <span class="unit">辆</span>
              </div>
              <div class="percent">
                <div class="num">{{ (aheadRank[2]||{}).percent }}</div>
              <!--            <span class="rank">1</span>-->
              <!--            <div class="down"></div>-->
              </div>
            </div>
        ...
        <div class="containerCenter">
          <div class="listCar" v-for="(item,index) in behindRank" :key="item.rank">
            ...
            <div class="text">{{ item.vehicleName.substring(0,4) }}</div>
            <div class="rightCar">
  ...
  methods: {
    ...
    initCarRank () {
      return new Promise((resolve, reject) => {
        ...
        this.$store.dispatch('CarRankRequest', params).then(res => {
          resolve()
          this.showLoading = false
          this.aheadRank = res.slice(0, 3)
          this.behindRank = res.slice(3, res.length)
        }).catch(err => {
          resolve()
          console.log(err)
          this.showLoading = false
