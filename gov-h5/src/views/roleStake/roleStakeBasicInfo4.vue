  ...
  <div>
    <div class="registerTop">
      ...
      <van-row>
        ...
        <mt-index-list v-if="barList1.length>0">
          ...
          <mt-index-section v-for="item in barList1"
                            :index="item"
                            :key="item">
            <mt-cell v-for="(car,index) in carEnterprises[item]"
              ...
              <div class="carList">
                <!-- <img width="42px"
                     ...
                     onerror="javascript:this.src='home/operator/operator_default.png';"
                     class="carCompanyImg" /> -->
                <span class="carCellText">{{ car.stationName }}</span>
...
export default {
  ...
  methods: {
    ...
    initCarEnterprises () {
      ...
      this.$store.dispatch('IndexList', parsms).then(res => {
        var wordLetter = res.wordLetter

        // this.carHotReg = res.hot
        // console.log(this.hotCarCompany)

        this.carEnterprises = wordLetter
        this.barList1 = res.firstLetter
