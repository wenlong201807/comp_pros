  ...
  <div style="position:relative;">
    ...
    <div class="top_num">
      ...
      <a-row>
        ...
        <a-col :span="6"
               style="text-align:center;background: #EA6F5D;color:#fff;padding: 4px 0;">
          <h3 style="margin-bottom:0;margin-top:2px;font-weight:800;font-size:18px;color:#fff">总充电量（万千瓦·时）</h3>
          <countTo style="font-weight:800;font-size:22px"
                   :startVal="totalEleStart"
                   :endVal="totalEle"
                   useEasing
                   :duration="3000">
          </countTo>
      ...
      <a-row :gutter="24"
        ...
        <a-col :span="8"
          ...
          <a-popover title="充电站用途">
            <template slot="content">
              <p v-for="item in contentList"
                 :key="item.NAME">{{ item.NAME }}：{{ item.VALUE }}座</p>
            </template>
            <a-button type="primary"
                      style="height:100%;width:100%;">
              <a-statistic class="white_text"
                           title="充电站数量"
                           :value="stationNum">
        ...
        <a-col :span="8"
          ...
          <a-popover title="充电桩类型">
            <template slot="content">
              <p v-for="item in staketypeList"
                 :key="item.NAME">{{ item.NAME }}：{{ item.value }}个</p>
            </template>
            <a-button type="primary"
                      style="height:100%;width:100%;">
              <a-statistic class="white_text"
...
export default {
  ...
  methods: {
    initOperatortotalPower () {
      ...
      this.$store.dispatch('OperatorOperatorTotalPower', params).then(res => {
    ...
    initStationType () {
      ...
      this.$store.dispatch('OpeStationTypeNumber', parsms).then(res => {
    ...
    initStationNum () {
      ...
      this.$store.dispatch('OpeStakeNumber', parsms).then(res => {
    ...
    initStakeUsePower () {
      ...
      this.$store.dispatch('OperatorStakeUsePower', params).then(res => {
    ...
    initStakeStatusNumber () {
      ...
      this.$store.dispatch('OpeStakeStatusNumber', parsms).then(res => {
    ...
    initStaketypenumber () {
      ...
      this.$store.dispatch('OpeStakeTypeNumber', parsms).then(res => {
    ...
    initStatisticsEleTimes () {
      ...
      this.$store.dispatch('OpeStatisticsEleTimes', data).then(res => {
        if (res) {
          // 获取当前系统时间
    ...
    numberFlashTDay (res) {
      ...
      this.ele =
        parseInt(that.eleStart) + Math.ceil(parseInt(res[0].ELE) / 8640)
      this.dayTimesStart = this.dayTimes
      this.dayTimes =
        parseInt(that.dayTimesStart) + Math.ceil(parseInt(res[0].times) / 8640)
    },
    ...
    initStatisticsEleTimesTotal () {
      ...
      this.$store.dispatch('OpeStatisticsEleTimes', data).then(res => {
        // this.totalTimes = parseInt((parseInt(res[0].times) / 10000).toFixed(2))
        // this.totalEle = parseInt((parseInt(res[0].ELE) / 10000).toFixed(2))
        var min = new Date().getMinutes()
        var hours = new Date().getHours()
        var seconds = new Date().getSeconds()
        var k = (hours * 60 * 60 + min * 60 + seconds) / 10
        ...
        that.totalEle = parseInt(
          (parseInt(that.totalEle) + parseInt(k * (res[0].ELE / 8640))).toFixed(
            0
          )
        )
