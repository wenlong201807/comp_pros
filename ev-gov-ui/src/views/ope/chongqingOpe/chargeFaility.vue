<template>
  <div>
    <a-card style="width: 100%; margin-bottom:24px">
      ...
      <a-select :defaultValue="defaultYear"
                style="width: 120px"
      ...
      <a-row>
        <a-col :span="8">
          <div style="height: 462px; padding: 15px; border-right: 1px solid #e7eaec;">
            <div style="background:url(/imageUrl/stationnum.png) no-repeat 20px 0px; margin-left:7%; background-size: contain;">
              <div style="font-size:20px;margin-left:40%;margin-top:30px;margin-bottom:50px">
                <div style="font-size:15px;"><b>充电站总数:</b></div>
                <div>
                  <span style="color:#00caaf;margin-left:0px;font-size: 25px;"><b>{{ stationNum }}</b></span><span style="font-size:15px;margin-left:0px;float:right;margin-right: 20%;margin-top: 10px;"><b>座</b></span>
                </div>
              </div>
            </div>
            <div style="background:url(/imageUrl/stakenum.png) no-repeat 20px 0px; margin-left:7%; background-size: contain;">
              <div style="font-size:20px;margin-left:40%;margin-top:30px;margin-bottom:50px">
                <div style="font-size:15px;"><b>充电桩总数:</b></div>
                <div>
                  <span style="color:#00caaf;margin-left:0px;font-size: 25px;"><b>{{ stakeNum }}</b></span><span style="font-size:15px;margin-left:0px;float:right;margin-right: 20%;margin-top: 10px;"><b>个</b></span>
                </div>
              </div>
            </div>
            <div style="background:url(/imageUrl/stakegl.png) no-repeat 20px 0px; margin-left:7%; background-size: contain;">
              <div style="font-size:20px;margin-left:40%;margin-top:30px;margin-bottom:50px">
                <div style="font-size:15px;"><b>总额定功率:</b></div>
                <div>
                  <span style="color:#00caaf;margin-left:0px;font-size: 25px;"><b>{{ ratingPower }}</b></span><span style="font-size:15px;margin-left:0px;float:right;margin-right: 20%;margin-top: 10px;"><b>千瓦</b></span>
                </div>
              </div>
            </div>
            <div style="background:url(/imageUrl/stakegl2.png) no-repeat 20px 0px; margin-left:7%; background-size: contain;">
              <div style="font-size:20px;margin-left:40%;margin-top:30px;margin-bottom:50px">
                <div style="font-size:15px;"><b>实时占用功率:</b></div>
                <div>
                  <span style="color:#00caaf;margin-left:0px;font-size: 25px;"><b>{{ nowPower }}</b></span><span style="font-size:15px;margin-left:0px;float:right;margin-right: 20%;margin-top: 10px;"><b>千瓦</b></span>
                </div>
              </div>
            </div>
          </div>
        ...
        <a-col :span="16">
          <div style="height: 231px; padding: 15px; border-bottom: 1px solid #e7eaec;"
               id="Chart1"></div>
        </a-col>
        <a-col :span="16">
          <div style="height: 231px; padding: 15px;"
               id="Chart2"></div>
        </a-col>
      </a-row>
    </a-card>
    ...
    <a-row :gutter="16"
      ...
      <a-col :span="8"
        ...
        <a-card :bordered="false"
                ...
                class="lastCard">
          <a-select defaultValue="充电设施建设规模排名"
                    style="width: 190px;"
          ...
          <div style="height: 350px;">
            <div style="height: 350px;">
              <div class="ulist">
                <div v-for="(item, index) in rankList.name"
                     :key="item"
                  ...
                  <div v-if="rankType == 1"
                    ...
                    <a-statistic :value="rankList.value[index]"
                                 style="display: inline-block;width:50px;" :valueStyle="{fontSize:'16px'}" /><span style="display: inline-block;text-align:right;width:40px;">个</span></div>
                  <div v-if="rankType == 2"
                       style="margin-left:20px; display:inline-block;">
                    <a-statistic :value="rankList.value[index]"
                                 style="display: inline-block;width:50px;" :valueStyle="{fontSize:'16px'}" /><span style="display: inline-block;text-align:right;width:40px;">千瓦</span></div>
                  <div v-if="rankType == 3"
                       style="margin-left:20px; display:inline-block;">
                    <a-statistic :value="rankList.value[index]"
                                 style="display: inline-block;width:50px;" :valueStyle="{fontSize:'16px'}" /><span style="display: inline-block;text-align:right;width:50px;">千瓦时</span></div>
                </div>
              </div>
            </div>
          </div>
...
export default {
  ...
  methods: {
    initStakeUsePower () {
      ...
      this.$store.dispatch('OpeStakeUsePower', params).then(res => {
    ...
    initNumber () {
      ...
      this.$store.dispatch('OpeNumber', params).then(res => {
    ...
    initOperatortotalPower () {
      ...
      this.$store.dispatch('OpeOperatorTotalPower', params).then(res => {
    ...
    loadData () {
      ...
      this.$store.dispatch('OpeTimesAndEleChange', params).then(data => {
        var key = data.name
        var ele = data.ele
    ...
    getdata () {
      ...
      this.$store.dispatch('OpeRealPowerHistory', params).then(data => {
        var key = data.name
      ...
      this.$store.dispatch('OpeStakeUseType', stakeParams).then(data => {
        var key = data.keys
      ...
      this.$store.dispatch('OpeStakeCategory', stakeParams).then(data => {
        var key = data.name
        var value = data.value
    ...
    getRank () {
      ...
      this.$store.dispatch('OpeStakeNumRank', rankParams).then(data => {
        that.rankList = data
      }).catch(error => {
        console.log(error)
      })
    ...
    changeRankMonth () {
      ...
      this.$store.dispatch('OpeStationIncrease', params).then(data => {
        var key = data.name
        var value = data.value
        var valueList = []
        for (var i in value) {
      ...
      this.$store.dispatch('OpeStakeIncrease', params).then(data => {
        var key = data.name
        var value = data.value
        var valueList = []
        for (var i in value) {
