    ...
    <a-card>
      <template slot="title">
        <span style="margin-left: 12px">
          <div class="titleFlag"></div>接入车辆状态统计
        </span>
      </template>
      <template slot="extra">
        ...
        <a-dropdown>
          ...
          >
            <a-checkbox-group
              ...
              :defaultValue="['name','startStatus','errorStatus','invalidStatus','unactivatedStatus','outlineStatus','endStatus','otherStatus','sum']"
            >
              <a-row>
                <a-col
...
export default {
  data () {
    return {
      ...
      columns: [
        ...
        {
          title: '未激活',
          dataIndex: 'unactivatedStatus'
        },
        {
          title: '离线',
          dataIndex: 'outlineStatus'
        },
        {
          title: '熄火',
          dataIndex: 'endStatus'
  ...
  methods: {
    ...
    handleTableChange(pagination, filters, sorter) {
      ...
      this.$store.dispatch("JoinUpStatusList", data).then(res => {
    ...
    expTable(){
      ...
      this.$store.dispatch("JoinUpStatusExp", data).then(res => {
        const blob = new Blob([res],{type:'application/vnd.ms-excel'})
        let fileName = '接入车辆状态统计.xls'
